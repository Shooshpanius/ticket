...
class TicketMailer < ActionMailer::Base
  ...
  def send_new_group_ticket_to_rctp_email(mail_data)
    ...
    mail(to: mail_data[:user_email], subject: subj ) if mail_data[:user_email].strip != ""
  end

  def send_new_group_ticket_to_sndr_email(mail_data)
    @mail_data = mail_data
    subj = "Заявка \''" + mail_data[:ticket_topic] + "\' зарегистрирована"
    mail(to: mail_data[:initiator_email], subject: subj ) if mail_data[:initiator_email].strip != ""
  end
  ...
  def send_new_comment_email(mail_data)
    ...
    mail(to: mail_data[:rcpt_email], subject: subj ) if mail_data[:rcpt_email].strip != ""
  end
